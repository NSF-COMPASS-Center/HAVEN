...
#SBATCH -A seqevol
#SBATCH -N1 
#SBATCH -t 50:00:00 # n hours

#SBATCH -p a100_normal_q
#SBATCH --gres=gpu:1
#SBATCH --ntasks-per-node=4
#SBATCH --mem-per-cpu=256G # Yeah, ik making all those permutations costs a lot of memory, which is the required format for TF, unless I want to alter the TF libs, this is fine
...
CONFIG_FILE=$CONFIG_DIR/hepTrainLang_Orf1.yaml
...
python $SCRIPT_LOCATION -c $CONFIG_FILE > $RESULTS_DIR/hep_lang_train_orf1.$(date +%Y_%b_%d_%H_%M).log 2>&1
