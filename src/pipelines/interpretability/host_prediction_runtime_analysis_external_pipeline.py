...
def execute(config):
    ...
    for model in models:
        ...
        df = dataset_utils.read_dataset(input_dir, [input_file_name], cols=[id_col, sequence_col, label_col])
        ...
        result_df = result_df.set_index(id_col).join(df[metadata_cols].set_index(id_col), how="left").reset_index()
