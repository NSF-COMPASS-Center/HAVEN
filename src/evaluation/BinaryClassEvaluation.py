...
class BinaryClassEvaluation(EvaluationBase):
    ...
    def compute_f1(self, df_itr):
        y_pred = self.convert_probability_to_prediction(df_itr)
        return f1_score(y_true=df_itr[self.y_true_col].values, y_pred=y_pred)

    def compute_auroc(self, df_itr):
        return roc_auc_score(y_true=df_itr[self.y_true_col].values, y_score=df_itr["1"].values, average="macro")

    def compute_auprc(self, df_itr):
        return average_precision_score(y_true=df_itr[self.y_true_col].values, y_score=df_itr["1"].values, average="macro")
