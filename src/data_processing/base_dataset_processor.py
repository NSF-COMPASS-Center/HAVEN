...
# The sequences will be joined back and compiled into one dataset at a later stage
def get_metadata_from_uniprot(input_file_path, output_file_path, id, query_uniprot):
    ...
    if Path(output_file_path).is_file():
        df_host = pd.read_csv(output_file_path, on_bad_lines=None, converters={2: literal_eval},
                              names=[id, TAX_ID, HOST_TAX_IDS, EMBL_REF_ID])
        df_host = df_host[[TAX_ID, id]]
        ...
        processed_hosts = list(df_host[id].unique())
        df = df[~df[id].isin(processed_hosts)]
    ...
    cpu_pool = Pool(N_CPU)
    cpu_pool.starmap(get_uniprot_metadata, zip(dfs, repeat(output_file_path), repeat(id), repeat(query_uniprot)))
...
# to the output file
def get_uniprot_metadata(df, output_file_path, id, query_uniprot):
    # get virus hosts
...
# Use multiprocessing to speed up the process
def get_virus_hosts_from_embl(input_file_path, embl_mapping_filepath, output_file_path, id):
...
def get_virus_metadata(input_file_path, taxon_metadata_dir_path, output_file_path, id):
    print("START: Retrieving virus and virus host metadata using pytaxonkit")
    # Set TAXONKIT_DB environment variable
    os.environ[TAXONKIT_DB] = taxon_metadata_dir_path
...
def join_metadata_with_sequences_data(input_file_path, sequence_data_file_path, output_file_path, id):
    ...
    merged_df = pd.merge(metadata_df, sequence_data_df[[id, SEQUENCE]], how="left", on=id)
