...
def execute(input_settings, output_settings, classification_settings):
    ...
    for input in inputs:
        ...
        index_label_map, train_dataset_loader = nn_utils.get_dataset_loader(seq_input_dir, input, sequence_settings,
                                                                            label_settings, dataset_type="train")
        index_label_map, test_dataset_loader = nn_utils.get_dataset_loader(seq_input_dir, input, sequence_settings,
        ...
        for model in models:
            ...
            if "tf-gnn" in model_name:
                ...
                tf_model = transformer.get_transformer_model(model).to(nn_utils.get_device())
                gnn_model = gnn_pipeline.get_gnn_model(model).to(nn_utils.get_device())

                result_df = run_models(tf_model, gnn_model, train_dataset_loader, test_dataset_loader, model["loss"],
...
def run_models(tf_model, gnn_model, train_dataset_loader, test_dataset_loader, loss, n_epochs, model_name, mode):
    tbw = SummaryWriter()
    criterion = nn_utils.get_criterion(loss)
    optimizer = torch.optim.Adam(tf_model.parameters(), lr=1e-4, weight_decay=1e-4)
