...
def compute_kmer_features(df, k, id_col, sequence_col, label_col, kmer_keys=None, kmer_prevalence_threshold=0.1):
    if kmer_keys and len(kmer_keys) > 0:
        print(f"Using provided {len(kmer_keys)} kmer_keys = {kmer_keys}")
    else:
        print("Computing kmer_keys")
        kmer_keys = get_kmer_keys(df, k, sequence_col, kmer_prevalence_threshold)
...
def get_kmer_keys(dataset, k, sequence_col, kmer_prevalence_threshold=0.1):
    print(f"kmer_prevalence_threshold = {kmer_prevalence_threshold}")
    ...
    filter_threshold = len(sequences) * kmer_prevalence_threshold  # default = atleast prevalence in 10% of the total number of sequences
    kmers_filtered = set()
