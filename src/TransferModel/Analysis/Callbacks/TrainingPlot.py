...
class TrainingPlot(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if len(self.lossLogs['Training Loss']) > 1:
            Visualization.plot_metrics(self.accLogs, f"{self.figDir}/ACC_{self.date}", metricName="Accuracy")
            Visualization.plot_metrics(self.lossLogs, f"{self.figDir}/CE_{self.date}", metricName="Cross entropy")
